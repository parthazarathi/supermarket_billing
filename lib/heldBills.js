...
function recallHeldBill(heldId, user = null) {
  return withTransaction((db) => {
    ...
    // Cashiers may only recall their own held bills; manager/admin may recall any
    const heldOwner = result[0].user_id;
    if (user && user.role === 'cashier' && heldOwner !== user.id) {
      throw new Error('You can only recall your own held bills');
    }
