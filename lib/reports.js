...
function outstandingSummary(type) {
  ...
  const returnsClause = type === 'supplier'
    ? '- COALESCE((SELECT SUM(pr.total - pr.refund_amount) FROM purchase_returns pr WHERE pr.party_id = p.id), 0)'
    : `- COALESCE((SELECT SUM(sr.total - sr.refund_amount) FROM sale_returns sr
        JOIN invoices ri ON sr.invoice_id = ri.id WHERE ri.party_id = p.id), 0)`;
  const r = qOne(`
    SELECT COUNT(*) as parties, COALESCE(SUM(due), 0) as total FROM (
      SELECT p.id, (
        ...
        ${returnsClause}
        - COALESCE((SELECT SUM(amount) FROM payments WHERE party_id = p.id
          AND (ref_type = 'standalone' OR (ref_type = '' AND note NOT LIKE 'Invoice %'))), 0)
      ) as due
