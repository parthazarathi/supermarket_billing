...
function pageParams(query = {}, defaultPerPage = 50) {
  ...
  const perPage = Math.min(1000, Math.max(1, parseInt(query.per_page, 10) || defaultPerPage));
...
// LIKE-pattern parameter: escapes %, _ and \ so they match literally.
// Queries must use `... LIKE ? ESCAPE '\'` for the escapes to apply.
