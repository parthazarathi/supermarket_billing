...
function completeSale(cart, options = {}) {
  ...
  let paidAmount = paid === null ? total : parseFloat(paid);
...
function recordInvoicePayment(invoiceId, amount, method = 'Cash', userId = null) {
  return withTransaction((db) => {
    ...
    if (invoice.party_id) {
      const paymentStmt = db.prepare("INSERT INTO payments (party_id, amount, method, note, created_at, user_id, direction) VALUES (?, ?, ?, ?, ?, ?, 'in')");
      paymentStmt.run([
        invoice.party_id,
        parseFloat(amount),
        method,
