...
function saveItem(data, itemId = null) {
  ...
  const salePrice = isNaN(rawSale) || rawSale === 0 ? purchasePrice * 1.2 : rawSale;

  if (purchasePrice < 0 || salePrice < 0) {
    throw new Error('Purchase price and sale price cannot be negative');
  }
  ...
  const fields = {
    ...
    gst_percent: parseFloat(data.gst_percent) || 18,
    ...
    try {
      if (itemId) {
        db.run(`
          ...
          gst_percent=${fields.gst_percent}, purchase_price=${fields.purchase_price},
        ...
        db.run(`
          INSERT INTO items (code, name, category, hsn, gst_percent, purchase_price, sale_price,
            stock, unit, low_stock, created_at, updated_at)
          VALUES ('${fields.code}', '${fields.name}', '${fields.category}', '${fields.hsn}', ${fields.gst_percent},
          ${fields.purchase_price}, ${fields.sale_price}, ${fields.stock}, '${fields.unit}', ${fields.low_stock},
          '${now}', '${now}')
        `);
...
function seedItemsFromJson() {
  ...
  return withTransaction((db) => {
    for (const [code, info] of Object.entries(products)) {
      ...
      try {
        db.run(`
          INSERT OR IGNORE INTO items (code, name, category, hsn, gst_percent, purchase_price, sale_price,
            stock, unit, low_stock, created_at, updated_at)
          VALUES ('${safeCode}', '${safeName}', '${safeCategory}', '', 18, ${Math.round(price * 0.7 * 100) / 100}, ${price},
