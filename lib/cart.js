...
function calculateCartTotals(cart, billDiscount = 0) {
  ...
  // Items marked 0% (or with no GST rate) are billed at the shop's default
  // GST rate - the tax is computed on the item's taxable value like any
  // other line.
  const defaultGst = parseFloat(getSetting('default_gst', '18')) || 0;
  const items = [];
  let subtotal = 0;
  let tax = 0;
  ...
  for (const [code, entry] of Object.entries(cart)) {
    ...
    const gstPercent = isFinite(gstRaw) && gstRaw > 0 ? gstRaw : defaultGst;
