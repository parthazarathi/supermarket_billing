...
// settingsOverride lets the gateway render from a snapshot without a local
// DB; one-arg calls read settings from the DB as before. The ./settings
// require stays inside the branch so gateway use never loads sql.js.
function generateInvoicePDF(invoice, settingsOverride) {
  return new Promise((resolve, reject) => {
    try {
      const settings = settingsOverride || require('./settings').getSettings();
