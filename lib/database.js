    ...
    try {
      ...
      const candidates = fs.existsSync(backupDir)
        ? fs.readdirSync(backupDir)
          .filter((f) => /^(MartPOS-backup|pre-restore|pre-migration)-.+\.db$/i.test(f))
          .sort()
          .reverse()
        : [];
...
async function initDatabase() {
  ...
  if (fs.existsSync(dbPath)) {
    ...
    if (fromVersion < SCHEMA_VERSION) {
      // Schema-changing migrations are about to run - keep a file-level
      // copy of the untouched database first so an app update can never
      // cost customer data.
      backupBeforeMigration(dbPath, fromVersion);
    }
    runMigrations();
  } else {
  ...
  // Stable installation id, generated once and kept in settings. Used for
  // support diagnostics - never a tenant identity (the gateway derives that
  // from the device token).
  try {
    db.run("INSERT OR IGNORE INTO settings (key, value) VALUES ('installation_id', ?)", [crypto.randomUUID()]);
    scheduleSave();
  } catch (e) {
    console.error('Could not ensure installation_id:', e.message);
  }
...
function createTables() {
  const createTablesSQL = `
    ...
    -- Persistent offline outbox for the Meta/gateway pipeline (schema v3).
    -- whatsapp_log above stays for history; new sends are tracked here.
    ...
    -- AI Store Manager audit trail: who asked what, which read-only tools
    -- ran, and the outcome. Credentials and customer PII are never logged.
    ...
    CREATE INDEX IF NOT EXISTS idx_wa_msg_invoice ON whatsapp_messages(invoice_id);
    CREATE INDEX IF NOT EXISTS idx_wa_msg_phone ON whatsapp_messages(normalized_phone);
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_wa_msg_remote
      ON whatsapp_messages(remote_id) WHERE remote_id IS NOT NULL AND remote_id != '';
    CREATE INDEX IF NOT EXISTS idx_wa_queue_due ON whatsapp_queue(status, next_retry_at);
    CREATE INDEX IF NOT EXISTS idx_wa_queue_invoice ON whatsapp_queue(invoice_id);
...
// PRAGMA user_version of the just-opened in-memory database (0 when unread).
...
// Copies pos.db into backups/ before schema migrations mutate it. Copies are
// named pre-migration-v<from>-to-v<to>-<stamp>.db and pruned to the newest
// few - they double as recovery sources in recoverDataFiles above.
const PREMIGRATION_KEEP = 5;
function backupBeforeMigration(dbPath, fromVersion) {
  try {
    const dir = path.join(path.dirname(dbPath), 'backups');
    fs.mkdirSync(dir, { recursive: true });
    const stamp = new Date().toISOString().replace(/[:.]/g, '-');
    const dest = path.join(dir, `pre-migration-v${fromVersion}-to-v${SCHEMA_VERSION}-${stamp}.db`);
    fs.copyFileSync(dbPath, dest);
    console.log(`Pre-migration backup written: ${path.basename(dest)}`);
    const old = fs.readdirSync(dir)
      .filter((f) => /^pre-migration-.+\.db$/i.test(f))
      .sort()
      .reverse()
      .slice(PREMIGRATION_KEEP);
    for (const f of old) {
      try { fs.unlinkSync(path.join(dir, f)); } catch (_) { /* best effort */ }
    }
  } catch (e) {
    // A failed backup must not block startup - runMigrations is idempotent.
